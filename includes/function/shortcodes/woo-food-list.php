<?php
/**
 * This is the food list shortcode file.
 *
 * @package woorestaurant
 */

/**
 * It is the shortcode list function.
 *
 * @param array $atts it is the parameter.
 */
function woo_restaurant_shortcode_list( $atts ) {
	if ( phpversion() >= 7 ) {
		$atts = (array) $atts;
	}
	...
	$posttype = isset( $atts['posttype'] ) && '' != $atts['posttype'] ? $atts['posttype'] : 'wr_food';
	...
	$showloc        = isset( $atts['showlocx'] ) ? $atts['showlocx'] : '';
	...
	if ( 'show' == $menu_filter && 'left' == $menu_pos ) {
	...
	?>
	<div class="wrs-fdlist list-layout <?php echo esc_attr( $class ); ?>" id ="<?php echo esc_attr( $id ); ?>">
		<?php
		...
		<?php
		if ( 'show' == $menu_filter ) {
			woo_restaurant_search_form_html( $cat, $order_cat, $menu_pos, $active_filter, $filter_style, $hide_ftall, $show_count );
		}
		?>
			...
			?>
				...
				while ( $the_query->have_posts() ) {
					...
					if ( 'yes' == $disable_is_visible || $product && $product->is_visible() ) {
						...
						woore_custom_color( 'list', $style, 'ctc-' . esc_attr( $id ) . '-' . esc_attr( get_the_ID() ) );
						?>
							<div class="exp-arrow" >
								<?php
...
add_shortcode( 'woores_view_list', 'woo_restaurant_shortcode_list' );
add_action( 'after_setup_theme', 'wr_reg_wfood_list_vc' );
/**
 * To register food list.
 */
function wr_reg_wfood_list_vc() {
	if ( function_exists( 'vc_map' ) ) {
		vc_map(
			array(
				'name'     => esc_html__( 'Food List', 'woorestaurant' ),
				'base'     => 'woores_view_list',
				'class'    => '',
				'icon'     => 'icon-grid',
				'controls' => 'full',
				...
				'params'   => array(
					...
					array(
						...
						'param_name'  => 'column',
						'value'       => array(
							esc_html__( '1 column', 'woorestaurant' )  => '',
					...
					array(
						...
						'type'        => 'dropdown',
						'class'       => '',
						'heading'     => esc_html__( 'Menu filter Position', 'woorestaurant' ),
						'param_name'  => 'menu_pos',
						'value'       => array(
							esc_html__( 'Top', 'woorestaurant' )  => 'top',
							esc_html__( 'Left', 'woorestaurant' ) => 'left',
						),
						'description' => esc_html__( 'Select posstion of menu filter', 'woorestaurant' ),
					),
					array(
						'admin_label' => true,
						'type'        => 'dropdown',
