<?php
/**
 * This is the Restaurant Shortcode Table file.
 *
 * @package woorestaurant
 */

/**
 * This is the Shortcode table function.
 *
 * @param array $atts it is the attribute parameter.
 */
function woo_restaurant_shortcode_table( $atts ) {
	if ( phpversion() >= 7 ) {
		$atts = (array) $atts;
	}
	...
	global $id, $count, $posts_per_page, $number_excerpt, $img_size, $hide_atc;
	// phpcs:ignore
	...
	if ( 'yes' == $hide_ftall && '' == $active_filter ) {
		if ( '' != $cat ) {
			$cats          = explode( ',', $cat );
	...
	$args = woo_restaurant_query( $posttype, $posts_per_page, $order, $orderby, $cat, $tag, $taxonomy, $meta_key, $ids, $meta_value, '', '', $active_filter, $featured );
	global $the_query;
	...
	?>
	<div class="wrs-fdlist table-layout 
	<?php
	echo esc_attr( $class );
	if ( '1' == $live_sort ) {
		echo ' table-lv-sort';
	...
	" id ="<?php echo esc_attr( $id ); ?>">
		...
		woo_restaurant_select_location_html( $locations );
		if ( 'show' == $menu_filter ) {
		...
		<?php } ?>
		<table class="wrsfd-table-<?php echo esc_attr( $style ); ?> <?php
		if ( '0' != $number_excerpt ) {
			echo 'wrsfd-non-showdes';
		}
		?>
		">
			<?php if ( 1 == $style ) { ?>
			<thead>
				<tr>
					<th><?php echo esc_html__( 'Image', 'woorestaurant' ); ?></th>
					<th class="exp-sort wrs-fd-name">
						<span class ="wrsfd-hide-screen"><?php echo esc_html__( 'Name', 'woorestaurant' ); ?></span>
						<span class="wrsfd-hide-mb  wrs-detail"><?php echo esc_html__( 'Detail', 'woorestaurant' ); ?></span>
					</th>
					<?php if ( '0' != $number_excerpt ) { ?>
					<th class="wrsfd-hide-screen wrs-fd-table-des"><?php echo esc_html__( 'Description', 'woorestaurant' ); ?></th>
					<?php } ?>
					<th class="exp-sort wrsfd-hide-screen wrsfd-hide-tablet wrs-fd-category"><?php echo esc_html__( 'Menu', 'woorestaurant' ); ?></th>
					<th class="exp-sort wrsfd-hide-screen wrsfd-price"><?php echo esc_html__( 'Price', 'woorestaurant' ); ?></th>
					<?php if ( 'yes' != $hide_atc ) { ?>
						<th class="wrs-fd-table-order"></th>
					<?php } ?>
				</tr>
			</thead>
			<?php } ?>
...
add_shortcode( 'woores_view_table', 'woo_restaurant_shortcode_table' );
add_action( 'after_setup_theme', 'wr_reg_wf_table_vc' );
/**
 * Register table.
 */
function wr_reg_wf_table_vc() {
	if ( function_exists( 'vc_map' ) ) {
		vc_map(
			array(
				'name'     => esc_html__( 'Food Table', 'woorestaurant' ),
				'base'     => 'woores_view_table',
				'class'    => '',
				'icon'     => 'icon-grid',
				'controls' => 'full',
				...
				'params'   => array(
					...
					array(
						...
						'type'        => 'dropdown',
						'class'       => '',
						'heading'     => esc_html__( 'Live Sort', 'woorestaurant' ),
						'param_name'  => 'live_sort',
						'value'       => array(
							esc_html__( 'No', 'woorestaurant' )  => '',
							esc_html__( 'Yes', 'woorestaurant' ) => '1',
						),
						'description' => esc_html__( 'Enable Live Sort', 'woorestaurant' ),
					),
					array(
						'admin_label' => true,
						'type'        => 'dropdown',
