...
window.WRMB2.charcounter = window.WRMB2.charcounter || {};

( function (window, document, $, wrmb, counter ) {
	'use strict';

	if ( ! wp.utils || ! wp.utils.WordCounter ) {
		return wrmb.log( 'Cannot find wp.utils!' );
	}
	...
	counter.updateCounter = function ( field_id ) {
		...
		var text     = wysiwyg ? instance.editor.getContent( { format : 'raw' } ) : wrmb.$id( field_id ).val().trim();
		...
		// Over maximum?
		instance.$el.parents( '.wrmb2-char-counter-wrap' )[ exceeded ? 'addClass' : 'removeClass' ]( 'wrmb2-max-exceeded' );
	...
	/**
	 * Initializes all character counters. Hooked to wrmb_init.
	 *
	 * @since  2.7.0
	 *
	...
	/**
	 * Initializes after a new repeatable row has been added. Hooked to wrmb2_add_row
	 *
	 * @since  2.7.0
	 *
	...
	counter.addRow = function ( evt, $row ) {
		...
		$row.find( '.wrmb2-char-counter' ).each(
			function () {
	 ...
	 * Removes counters after a repeatable row has been removed. Hooked to wrmb2_remove_row.
	 *
	 * @since  2.7.0
	 *
	 * @return {void}
	 ...
	 * @return {void}
	 */
	counter.countWysiwyg = _.throttle(
		function ( evt ) {
	 ...
	 * @return {void}
	 */
	counter.countTextarea = _.throttle(
		function (evt) {
	...
	$( document )
		.on( 'wrmb_init', counter.initAll )
		.on( 'tinymce-editor-init', counter.initWysiwyg )
		.on( 'wrmb2_add_row', counter.addRow )
		.on( 'wrmb2_remove_row', counter.cleanCounters )
		.on( 'input keyup', '.wrmb2-count-chars', counter.countTextarea );

} )( window, document, jQuery, window.WRMB2, window.WRMB2.charcounter );
