...
window.WRMB2.wysiwyg = window.WRMB2.wysiwyg || {};

( function (window, document, $, wrmb, wysiwyg, undefined ) {
	'use strict';
	...
	function getGroupData( data ) {
		...
		if ( ! all[ groupid ] || ! all[ groupid ][ fieldid ] ) {
			...
			all[ groupid ][ fieldid ] = {
				template : wp.template( 'wrmb2-wysiwyg-' + groupid + '-' + fieldid ),
				defaults : {

					// Get the data from the template-wysiwyg initiation.
					mce : $.extend( {}, tinyMCEPreInit.mceInit[ 'wrmb2_i_' + groupid + fieldid ] ),
					qt  : $.extend( {}, tinyMCEPreInit.qtInit[ 'wrmb2_i_' + groupid + fieldid ] )
				}
			};
			// This is the template-wysiwyg data, and we do not want that to be initiated.
			delete tinyMCEPreInit.mceInit[ 'wrmb2_i_' + groupid + fieldid ];
			delete tinyMCEPreInit.qtInit[ 'wrmb2_i_' + groupid + fieldid ];
		}
	...
	function initOptions( options ) {
		var nameRegex = new RegExp( 'wrmb2_n_' + options.groupid + options.fieldid, 'g' );
		var idRegex   = new RegExp( 'wrmb2_i_' + options.groupid + options.fieldid, 'g' );
	...
	/**
	 * Initializes all group wysiwyg editors. Hooked to wrmb_init.
	 *
	 * @since  2.2.3
	 *
	 * @return {void}
	...
	wysiwyg.initAll = function () {
		...
		$( '.wrmb2-wysiwyg-placeholder' ).each(
			function () {
				$this = $( this );
				data  = $this.data();
		...
		if ( true === initiated ) {
			...
			$( document )
				.on( 'wrmb2_add_row', wysiwyg.addRow )
				.on( 'wrmb2_remove_group_row_start', wysiwyg.destroyRowEditors )
				.on( 'wrmb2_shift_rows_start', wysiwyg.shiftStart )
				.on( 'wrmb2_shift_rows_complete', wysiwyg.shiftComplete );
		}
	};

	/**
	 * Initiates wysiwyg editors in a new group row. Hooked to wrmb2_add_row.
	 *
	 * @since  2.2.3
	 *
	...
	/**
	 * Destroys wysiwyg editors in a group row when that row is removed. Hooked to wrmb2_remove_group_row_start.
	 *
	 * @since  2.2.3
	 *
	 * @param  {object} evt A jQuery-normalized event object.
	...
	wysiwyg.destroyRowEditors = function ( evt, $btn ) {
		wysiwyg.destroy( $btn.parents( '.wrmb-repeatable-grouping' ).find( '.wp-editor-area' ).attr( 'id' ) );
	};
	...
	/**
	 * Initializes editors for a new WRMB row.
	 *
	 * @since  2.2.3
	 *
	 * @param  {object} $row A jQuery dom element object for the group row.
	...
	wysiwyg.initRow = function ( $row, evt ) {
		...
		$row.find( '.wrmb2-wysiwyg-inner-wrap' ).each(
			function () {
				$toReplace = $( this );
				data       = $toReplace.data();
				defVal     = wrmb.getFieldArg( data.hash, 'default', '' );
				...
				data.value    = 'wrmb2_add_row' !== evt.type && $toReplace.find( '.wp-editor-area' ).length ? $toReplace.find( '.wp-editor-area' ).val() : defVal;
	...
	// Hook in our event callbacks.
	$( document ).on( 'wrmb_init', wysiwyg.initAll );

} )( window, document, jQuery, window.WRMB2, window.WRMB2.wysiwyg );
