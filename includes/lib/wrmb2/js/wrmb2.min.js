...
(function (window, document, $, wrmb, undefined) {
  ...
  wrmb.init = function () {
    ...
    $metabox
      .on("change", ".wrmb2_upload_file", function () {
  ...
  wrmb.resetTitlesAndIterator = function (evt) {
    ...
    $(".wrmb-repeatable-group.repeatable").each(function () {
      var $table = $(this);
      var groupTitle = $table.find(".wrmb-add-group-row").data("grouptitle");

      // Loop repeatable group table rows
      $table.find(".wrmb-repeatable-grouping").each(function (rowindex) {
        var $row = $(this);
  ...
  wrmb.setDefaults = function (evt, row) {
    $(wrmb.noEmpty, row).each(function () {
      var $el = $(this);
  ...
  wrmb.shiftRows = function (evt) {
    ...
    $from.find(wrmb.repeatEls).each(function () {
      var $element = $(this);
      var elType = $element.attr("type");
      var val;
    ...
    // And swap them all
    $goto.find(wrmb.repeatEls).each(function (index) {
      var $element = $(this);
      var elType = $element.attr("type");
      var val;
  ...
  wrmb.initPickers = function ($timePickers, $datePickers, $colorPickers) {
    wrmb.trigger("wrmb_init_pickers", {
      time: $timePickers,
      date: $datePickers,
      color: $colorPickers,
    });
    ...
    if ($repeatableGroups.length) {
      $repeatableGroups.sortable({
        ...
        stop: function (ev) {
          ...
          if ($group.closest(".wrmb2-id-woores-options").length) {
            $group.find(".wrmb-repeatable-grouping").each(function (i, row) {
              var $this_gr = $(this);
          ...
          } else if ($group.closest(".wrmb2-id-wrsfood-adv-timedeli").length) {
            $group.find(".wrmb-repeatable-grouping").each(function (i, row) {
              var $this_gr = $(this);
              $this_gr.find("input").each(function () {
                var $_name = $(this).attr("name");
                ...
                $_name = $_name.replace(
                  res[0],
                  "wooresfd_adv_timedeli[" + (iterator - 1)
                );
                $(this).attr("name", $_name);
              });
              $this_gr.find("select").each(function () {
            ...
            $group.find(".wrmb-repeatable-grouping").each(function (i, row) {
              ...
              $this_gr.find("input").each(function () {
                var $_name = $(this).attr("name");
                var res = $_name.split("][");
                var name_op = res[0] !== null ? res[0].split("[") : "";
                if (name_op != "" && name_op[0] !== null) {
                  $_name = $_name.replace(
                    res[0],
                    name_op[0] + "[" + (iterator - 1)
                  );
                }
                $(this).attr("name", $_name);
              });
