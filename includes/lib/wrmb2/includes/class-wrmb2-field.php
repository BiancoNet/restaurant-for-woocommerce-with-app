 ...
 * @method mixed fields()
 */
class WRMB2_Field extends WRMB2_Base {
	...
	/**
	 * All WRMB2_Field callable field arguments.
	...
	public function __construct( $args ) {
		...
		if ( ! empty( $args['group_field'] ) ) {
			...
			$this->wrmb_id     = $this->group->wrmb_id;
		} else {
			$this->object_id   = isset( $args['object_id'] ) && '_' !== $args['object_id'] ? $args['object_id'] : 0;
			$this->object_type = isset( $args['object_type'] ) ? $args['object_type'] : 'post';

			if ( isset( $args['wrmb_id'] ) ) {
				$this->wrmb_id = $args['wrmb_id'];
			}
		}
	...
	public function get_data( $field_id = '', $args = array() ) {
		 ...
		 * Returning a non 'wrmb2_field_no_override_val' value
		 ...
		 * }
		 *
		 * @param WRMB2_Field object $field This field object
		 */
		$data = apply_filters( 'wrmb2_override_meta_value', 'wrmb2_field_no_override_val', $this->object_id, $a, $this );

		/**
		 * Filter and parameters are documented for 'wrmb2_override_meta_value' filter (above).
		 *
		 * The dynamic portion of the hook, $field_id, refers to the current
		 * field id paramater. Returning a non 'wrmb2_field_no_override_val' value
		 * will effectively short-circuit the value retrieval.
		 *
		 * @since 2.0.0
		 */
		$data = apply_filters( "wrmb2_override_{$a['field_id']}_meta_value", $data, $this->object_id, $a, $this );

		// If no override, get value normally.
		if ( 'wrmb2_field_no_override_val' === $data ) {
			$data = 'options-page' === $a['type']
				? wrmb2_options( $a['id'] )->get( $a['field_id'] )
	...
	public function update_data( $new_value, $single = true ) {
		 ...
		 * @param WRMB2_Field object $field This field object
		 */
		$override = apply_filters( 'wrmb2_override_meta_save', null, $a, $this->args(), $this );

		/**
		 * Filter and parameters are documented for 'wrmb2_override_meta_save' filter (above).
		 *
		 ...
		 * @since 2.0.0
		 */
		$override = apply_filters( "wrmb2_override_{$a['field_id']}_meta_save", $override, $a, $this->args(), $this );
		...
		if ( 'options-page' === $a['type'] || empty( $a['id'] ) ) {
			return wrmb2_options( $a['id'] )->update( $a['field_id'], $a['value'], false, $a['single'] );
		}
	...
	public function remove_data( $old = '' ) {
		 ...
		 * @param WRMB2_Field object $field This field object
		 */
		$override = apply_filters( 'wrmb2_override_meta_remove', null, $a, $this->args(), $this );
		 ...
		 * @param WRMB2_Field object $field This field object
		 */
		$override = apply_filters( "wrmb2_override_{$a['field_id']}_meta_remove", $override, $a, $this->args(), $this );

		// If no override, remove as usual.
		if ( null !== $override ) {
			return $override;
		} elseif ( 'options-page' === $a['type'] || empty( $a['id'] ) ) {
			return wrmb2_options( $a['id'] )->remove( $a['field_id'] );
		}
	...
	public function sanitization_cb( $meta_value ) {
		...
		$sanitizer  = new WRMB2_Sanitize( $this, $meta_value );
		 ...
		 * @param object     $sanitizer  This `WRMB2_Sanitize` object
		 */
		$override_value = apply_filters( "wrmb2_sanitize_{$field_type}", null, $sanitizer->value, $this->object_id, $this->args(), $sanitizer );

		if ( null !== $override_value ) {
			return $override_value;
		}

		// Sanitization via 'WRMB2_Sanitize'.
	...
	public function save_field( $meta_value ) {
		...
		} elseif ( ! WRMB2_Utils::isempty( $new_value ) && $new_value !== $this->get_data() ) {
			$updated = $this->update_data( $new_value );
			$action  = 'updated';
		} elseif ( WRMB2_Utils::isempty( $new_value ) ) {
		 ...
		 * @param WRMB2_Field object $field    This field object
		 */
		do_action( 'wrmb2_save_field', $field_id, $updated, $action, $this );
		 ...
		 * @param WRMB2_Field object $field   This field object
		 */
		do_action( "wrmb2_save_field_{$field_id}", $updated, $action, $this );
	...
	public function repeatable_exception( $type ) {
		 ...
		 */
		$all_fields = array_merge( apply_filters( 'wrmb2_non_repeatable_fields', array() ), $internal_fields );
	...
	public function escaped_value( $func = 'esc_attr', $meta_value = '' ) {
		 ...
		 * @param array      $field_args The current field's arguments.
		 * @param object     $field      This `WRMB2_Field` object.
		 */
		$esc = apply_filters( "wrmb2_types_esc_{$field_type}", null, $meta_value, $this->args(), $this );
		if ( null !== $esc ) {
			return $esc;
		}
	...
	public function val_or_default( $meta_value ) {
		return ! WRMB2_Utils::isempty( $meta_value ) ? $meta_value : $this->get_default();
	}
	...
	public function field_timezone_offset() {
		return WRMB2_Utils::timezone_offset( $this->field_timezone() );
	}
	...
	public function get_timestamp_format( $format = 'date_format', $meta_value = 0 ) {
		...
		$meta_value = WRMB2_Utils::make_valid_time_stamp( $meta_value );
		if ( empty( $meta_value ) ) {
			return '';
		}
	...
	public function get_timestamp_from_value( $value ) {
		return WRMB2_Utils::get_timestamp_from_value( $value, $this->args( 'date_format' ) );
	}
	...
	public function render_field_callback() {
		...
		$types = new WRMB2_Types( $this );
		/**
		 * Hook before field row begins.
		 *
		 * @param WRMB2_Field $field The current field object.
		 */
		do_action( 'wrmb2_before_field_row', $this );
		 ...
		 * @param WRMB2_Field $field The current field object.
		 */
		do_action( "wrmb2_before_{$field_type}_field_row", $this );

		$this->peform_param_callback( 'before_row' );

		printf( "<div class=\"wrmb-row %s\" data-fieldtype=\"%s\">\n", esc_attr( $this->row_classes() ), esc_attr( $field_type ) );

		if ( ! $this->args( 'show_names' ) ) {
			echo "\n\t<div class=\"wpc-label wrmb-th\">\n";
			...
			if ( $this->get_param_callback_result( 'label_cb' ) ) {
				echo '<div class="wpc-label wrmb-th">', esc_attr( $this->peform_param_callback( 'label_cb' ) );
				if ( isset( $types->field->args['desc'] ) && ! empty( $types->field->args['desc'] ) ) {
					echo '<div class="wpc-desc"> ' . esc_attr( $types->field->args['desc'] ) . ' </div>';}
				echo '</div>';
			}
			$is_more = isset( $types->field->args['is_60'] ) && ! empty( $types->field->args['is_60'] ) ? 'wooressixty' : '';
			// phpcs:ignore
			echo ( "\n\t<div class=\"wpc-meta $is_more wrmb-td\">\n" );
		}
		 ...
		 * @param WRMB2_Field $field The current field object.
		 */
		do_action( "wrmb2_after_{$field_type}_field_row", $this );
		 ...
		 * @param WRMB2_Field $field The current field object.
		 */
		do_action( 'wrmb2_after_field_row', $this );
	...
	/**
	 * Defines the classes for the current WRMB2 field row
	 *
	 * @since  2.0.0
	...
	public function row_classes() {
		...
		$conditional_classes = array(
			'wrmb-type-' . str_replace( '_', '-', sanitize_html_class( $this->type() ) ) => true,
			'wrmb2-id-' . str_replace( '_', '-', sanitize_html_class( $this->id() ) ) => true,
			'wrmb-repeat'             => $this->args( 'repeatable' ),
			'wrmb-repeat-group-field' => $this->group,
			'wrmb-inline'             => $this->args( 'inline' ),
		...
		$added_classes      = $this->args( 'classes' );
		$added_classes_else = $this->get_param_callback_result( 'classes_cb' );
		if ( $added_classes ) {
			$added_classes = is_array( $added_classes ) ? implode( ' ', $added_classes ) : (string) $added_classes;
		} elseif ( $added_classes_else ) {
			$added_classes = is_array( $added_classes_else ) ? implode( ' ', $added_classes_else ) : (string) $added_classes_else;
		}
		 ...
		 * @param WRMB2_Field object $field   This field object
		 */
		return apply_filters( 'wrmb2_row_classes', implode( ' ', $classes ), $this );
	}
	...
	public function get_rest_value() {
		...
		$cb = $this->maybe_callback( 'rest_value_cb' );
		if ( $cb ) {
			add_filter( "wrmb2_get_rest_value_for_{$field_id}", $cb, 99 );
		}
		 ...
		 * @since 2.5.0
		 *
		 * @param mixed      $value The value from WRMB2_Field::get_data()
		 * @param WRMB2_Field $field This field object.
		 */
		$value = apply_filters( 'wrmb2_get_rest_value', $value, $this );
		 ...
		 * @param WRMB2_Field $field This field object.
		 */
		$value = apply_filters( "wrmb2_get_rest_value_{$field_type}", $value, $this );
		 ...
		 * @param WRMB2_Field $field This field object.
		 */
		return apply_filters( "wrmb2_get_rest_value_for_{$field_id}", $value, $this );
	}
	 ...
	 * @since  2.7.0
	 *
	 * @return WRMB2_Field|bool Supporting field object, if supported.
	 */
	...
	public function display_value_callback() {
		...
		$display    = new WRMB2_Field_Display( $this );
		$field_type = $this->type();
		 ...
		 * @param WRMB2_Field         $field      This field object.
		 * @param WRMB2_Field_Display $display    The `WRMB2_Field_Display` object.
		 */
		$pre_output = apply_filters( "wrmb2_pre_field_display_{$field_type}", null, $this, $display );

		if ( null !== $pre_output ) {
			echo esc_attr( $pre_output );
			return;
		}

		$this->peform_param_callback( 'before_display_wrap' );

		printf( "<div class=\"wrmb-column %s\" data-fieldtype=\"%s\">\n", esc_attr( $this->row_classes() ), esc_attr( $field_type ) );

		$this->peform_param_callback( 'before_display' );

		WRMB2_Field_Display::get( $this )->display();
	...
	public function add_js_dependencies( $dependencies = array() ) {
		...
		WRMB2_JS::add_dependencies( $dependencies );
	}
	...
	public function register_js_data() {
		if ( $this->group ) {
			WRMB2_JS::add_field_data( $this->group );
		}

		return WRMB2_JS::add_field_data( $this );
	}
	...
	public function hash_id() {
		if ( '' === $this->hash_id ) {
			$this->hash_id = WRMB2_Utils::generate_hash( $this->wrmb_id . '||' . $this->id() );
		}
	...
	/**
	 * Get WRMB2_Field default value, either from default param or default_cb param.
	 *
	 * @since  0.2.2
	 *
	...
	public function get_default() {
		...
		if ( null !== $default ) {
			return apply_filters( 'wrmb2_default_filter', $default, $this );
		}

		$param                 = is_callable( $this->args['default_cb'] ) ? 'default_cb' : 'default';
		$default               = $this->get_param_callback_result( $param );
		$this->args['default'] = $this->get_param_callback_result( $param );

		// Allow a filter override of the default value.
		return apply_filters( 'wrmb2_default_filter', $this->args['default'], $this );
	}
	...
	public function _set_field_defaults( $args ) {
		...
		/**
		 * Filter the WRMB2 Field defaults.
		 *
		 * @since 2.6.0
		 ...
		 * @param WRMB2_Field object $field    This field object.
		 */
		$defaults = apply_filters( 'wrmb2_field_defaults', $defaults, $args['id'], $args['type'], $this );

		// Set up blank or default values for empty ones.
		$args = wp_parse_args( $args, $defaults );

		/**
		 * Filtering the WRMB2 Field arguments once merged with the defaults, but before further processing.
		 *
		 * @since 2.6.0
		 * @param array             $args  Metabox field config array defaults.
		 * @param WRMB2_Field object $field This field object.
		 */
		$args = apply_filters( 'wrmb2_field_arguments_raw', $args, $this );
		...
		$method = $this->has_args_method( $args['type'] );
		if ( $method ) {
		...
		$with_supporting = array(
			// WRMB2_Sanitize::_save_file_id_value()/WRMB2_Sanitize::_get_group_file_value_array().
			'file'                             => '_id',
			// See WRMB2_Sanitize::_save_utc_value().
		...
		if ( ! empty( $args['repeatable'] ) ) {
			WRMB2_JS::add_dependencies( 'jquery-ui-sortable' );
		}

		/**
		 * Filter the WRMB2 Field arguments after processing.
		 *
		 * @since 2.6.0
		 * @param array             $args  Metabox field config array after processing.
		 * @param WRMB2_Field object $field This field object.
		 */
		return apply_filters( 'wrmb2_field_arguments', $args, $this );
	}
	...
	protected function set_field_defaults_group( $args ) {
		$args['options'] = wp_parse_args(
			$args['options'],
			array(
				'add_button'     => esc_html__( 'Add Group', 'wrmb2' ),
				'remove_button'  => esc_html__( 'Remove Group', 'wrmb2' ),
				'remove_confirm' => '',
			)
		);
	...
	protected function set_field_defaults_all_or_nothing_types( $args ) {
		...
		$args['show_option_none'] = true === $args['show_option_none'] ? esc_html__( 'None', 'wrmb2' ) : $args['show_option_none'];

		if ( null === $args['show_option_none'] ) {
			$off_by_default           = in_array( $args['type'], array( 'select', 'radio', 'radio_inline' ), true );
			$args['show_option_none'] = $off_by_default ? false : esc_html__( 'None', 'wrmb2' );
		}
	 ...
	 *                           arguments for WRMB2_Field.
	 *
	 * @return WRMB2_Field         The new WRMB2_Field instance.
	 */
	...
	/**
	 * Returns the WRMB2 instance this field is registered to.
	 *
	 * @since  2.2.2
	 *
	 * @return WRMB2|WP_Error If new WRMB2_Field is called without wrmb_id arg, returns error.
	 */
	public function get_wrmb() {
		if ( ! $this->wrmb_id ) {
			return new WP_Error( 'no_wrmb_id', esc_html__( 'Sorry, this field does not have a wrmb_id specified.', 'wrmb2' ) );
		}

		return wrmb2_get_metabox( $this->wrmb_id, $this->object_id, $this->object_type );
	}
