<?php
/**
 * It is the Type Wysiwyg file.
 *
 * @package woorestaurant
 */

/**
 * WRMB wysiwyg field type.
 *
 * @since  2.2.2
 *
 ...
 * @method string _desc()
 */
class WRMB2_Type_Wysiwyg extends WRMB2_Type_Textarea {
	...
	public function render( $args = array() ) {
		...
		if ( ! $field->group ) {
			...
			if ( $this->has_counter ) {
				$a['options']['editor_class'] = ! empty( $a['options']['editor_class'] )
					? $a['options']['editor_class'] . ' wrmb2-count-chars'
					: 'wrmb2-count-chars';
			}
		...
		$field->add_js_dependencies( 'wp-util', 'wrmb2-wysiwyg' );
		...
		return $this->rendered(
			sprintf(
				'<div class="wrmb2-wysiwyg-wrap">%s',
				parent::render(
					array(
						'class'         => 'wrmb2_textarea wrmb2-wysiwyg-placeholder',
	...
	/**
	 * To add wysiwyg template for group.
	 */
	public function add_wysiwyg_template_for_group() {
		...
		$options['textarea_name'] = 'wrmb2_n_' . $group_id . $field_id;

		// Initate the editor with special id/value/name so we can retrieve the options in JS.
		$editor = $this->get_wp_editor(
			array(
				'value'   => 'wrmb2_v_' . $group_id . $field_id,
				'id'      => 'wrmb2_i_' . $group_id . $field_id,
				'options' => $options,
			)
		);
		...
		$editor = str_replace(
			array(
				'wrmb2_n_' . $group_id . $field_id,
				'wrmb2_v_' . $group_id . $field_id,
				'wrmb2_i_' . $group_id . $field_id,
			),
			array(
				'{{ data.name }}',
				'{{{ data.value }}}',
		...
		echo '<script type="text/template" id="tmpl-wrmb2-wysiwyg-' . esc_attr( $group_id ) . '-' . esc_attr( $field_id ) . '">';
		// Need to wrap the template in a wrapper div w/ specific data attributes which will be used when adding/removing rows.
		echo '<div class="wrmb2-wysiwyg-inner-wrap" data-iterator="{{ data.iterator }}" data-groupid="' . esc_attr( $group_id ) . '" data-id="' . esc_attr( $field_id ) . '" data-hash="' . esc_attr( $hash ) . '">' . esc_attr( $editor ) . '</div>';
		echo '</script>';
	}
}
