 ...
 * @link      https://wrmb2.io
 */
abstract class WRMB2_Type_Base {

	/**
	 * The WRMB2_Types object
	 *
	 * @var WRMB2_Types
	 */
	public $types;
	 ...
	 */
	public function __construct( WRMB2_Types $types, $args = array() ) {
		$this->types      = $types;
	 ...
	 *
	 * @since  2.2.2
	 * @param  string|WRMB2_Type_Base $rendered Rendered output.
	 * @return string|WRMB2_Type_Base           Rendered output or this object.
	 */
	...
	public function parse_args( $element, $type_defaults, $type_overrides = array() ) {
		 ...
		 * @param array  $field             The `WRMB2_Field` object.
		 * @param object $field_type_object This `WRMB2_Types` object.
		 */
		$args = apply_filters( "wrmb2_{$element}_attributes", $args, $type_defaults, $this->field, $this->types );
	...
	/**
	 * Fall back to WRMB2_Types methods
	 *
	...
	public function __call( $method, $arguments ) {
		switch ( $method ) {
			...
			default:
				...
				throw new Exception( sprintf( esc_html__( 'Invalid %1$s method: %2$s', 'wrmb2' ), __CLASS__, esc_attr( $method ) ) );
		}
	}
	...
	public function __get( $field ) {
		switch ( $field ) {
			...
			default:
				/* translators: %1$s is replaced with the class, %1$s is replaced with the method */
