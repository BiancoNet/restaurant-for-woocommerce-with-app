<?php
/**
 * It is the Type Counter Base.
 *
 * @package woorestaurant
 */

/**
 * WRMB base field type
 *
 * @since  2.2.2
 *
 ...
 * @link      https://wrmb2.io
 */
abstract class WRMB2_Type_Counter_Base extends WRMB2_Type_Base {
	...
	public function char_counter_markup( $val ) {
		...
		switch ( $type ) {
			case 'words':
				$label = $char_max
					? $this->_text( 'words_left_text', esc_html__( 'Words left', 'wrmb2' ) )
					: $this->_text( 'words_text', esc_html__( 'Words', 'wrmb2' ) );
				break;
			default:
				$type  = 'characters';
				$label = $char_max
					? $this->_text( 'characters_left_text', esc_html__( 'Characters left', 'wrmb2' ) )
					: $this->_text( 'characters_text', esc_html__( 'Characters', 'wrmb2' ) );
				break;
		}

		$msg = $char_max
			? sprintf( '<span class="wrmb2-char-max-msg">%s</span>', $this->_text( 'characters_truncated_text', esc_html__( 'Your text may be truncated.', 'wrmb2' ) ) )
			: '';
		...
		$markup .= '<p class="wrmb2-char-counter-wrap">';
		$markup .= sprintf(
			'<label><span class="wrmb2-char-counter-label">%2$s:</span> <input id="%1$s" data-field-id="%3$s" data-counter-type="%4$s" %5$s class="wrmb2-char-counter" type="text" value="%6$s" readonly="readonly" style="%7$s"></label>%8$s',
	...
	public function maybe_update_attributes_for_char_counter( $attributes ) {
		...
		if ( $char_counter ) {
			$attributes['class'] = ! empty( $attributes['class'] ) ? $attributes['class'] . ' wrmb2-count-chars' : ' wrmb2-count-chars';
