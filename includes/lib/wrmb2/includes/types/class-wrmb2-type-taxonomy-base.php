<?php
/**
 * It is the Taxonomy Base file.
 *
 * @package woorestaurant
 */

/**
 * WRMB Taxonomy base field type
 *
 * @since  2.2.2
 *
 ...
 * @link      https://wrmb2.io
 */
abstract class WRMB2_Type_Taxonomy_Base extends WRMB2_Type_Multi_Base {
	...
	public function non_post_object_terms() {
		...
		$cache_key = "wrmb-cache-{$taxonomy}-{$object_id}";
	...
	public function get_terms() {
		...
		// phpcs:ignore
		return WRMB2_Utils::wp_at_least( '4.5.0' ) ? get_terms( $args ) : get_terms( $this->field->args( 'taxonomy' ), http_build_query( $args ) );
	}

	/**
	 * It is no terms result function.
	 *
	 * @param object $error it is the error object.
	 * @param string $tag it is the string.
	 */
	protected function no_terms_result( $error, $tag = 'li' ) {
		if ( is_wp_error( $error ) ) {
			...
			$message = $this->_text( 'no_terms_text', esc_html__( 'No terms', 'wrmb2' ) );
			$data    = '';
		}
	 ...
	 * @param  array|string $saved_terms Array of terms set to the object, or single term slug.
	 *
	...
	public function child_option_output( $terms, $saved ) {
		$output  = '<li class="wrmb2-indented-hierarchy"><ul>';
