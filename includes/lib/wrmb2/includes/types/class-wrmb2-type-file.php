 ...
 * @link      https://wrmb2.io
 */
class WRMB2_Type_File extends WRMB2_Type_File_Base {
	...
	public function render( $args = array() ) {
		...
		$this->args = $this->parse_args(
			'file',
			array(
				'class'           => 'wrmb2-upload-file wpc-settings-input',
				'id'              => $this->_id(),
				'name'            => $this->_name(),
		...
		$img_size_data = parent::get_image_size_data(
			$a['preview_size'],
			'large'
		);

		$this->args['img_size_data'] = $img_size_data;
		...
		// This is not elegant, but compensates for WRMB2_Types::_id
		...
		$output .= sprintf(
			'<input class="wrmb2-upload-button button-secondary " type="button" value="%1$s" />',
			esc_attr( $this->_text( 'add_upload_file_text', esc_html__( 'Add or Upload File', 'wrmb2' ) ) )
		);

		// $output .= $a['desc'];
		$output .= $this->get_id_field_output();

		$output .= '<div id="' . esc_attr( $field->id() ) . '-status" class="wrmb2-media-status">';
		if ( ! empty( $a['value'] ) ) {
	...
	public function get_file_preview_output() {
		...
		if ( $this->args['id_value'] ) {
			$image = wp_get_attachment_image(
				...
				null,
				array(
					'class' => 'wrmb-file-field-image',
				)
			);
		} else {
			$image = '<img style="max-width: ' . absint( $this->args['img_size_data']['width'] ) . 'px; width: 100%;" src="' . esc_url( $this->args['value'] ) . '" class="wrmb-file-field-image" alt="" />';
		}
	...
	public function get_id_field() {
		...
		if ( $this->args['value'] && ! $id_value ) {
			$id_value = WRMB2_Utils::image_id_from_url( esc_url_raw( $this->args['value'] ) );
		}
