<?php
/**
 * It is the WRMB2 Base file.
 *
 * @package woorestaurant
 */

/**
 * WRMB2 Base - Base object functionality.
 *
...
abstract class WRMB2_Base {
	...
	public function prop( $property, $fallback = null ) {
		if ( array_key_exists( $property, $this->{$this->properties_name} ) ) {
			...
			$this->{$this->properties_name}[ $property ] = $fallback;
			return $this->{$this->properties_name}[ $property ];
		}
	}
	...
	/**
	 * Get a new field object specific to this WRMB2 object.
	 *
	 * @since  2.2.0
	 * @param  array       $field_args  Metabox field config array.
	 * @param  WRMB2_Field $field_group (optional) WRMB2_Field object (group parent).
	 * @return WRMB2_Field WRMB2_Field object
	 */
	protected function get_new_field( $field_args, $field_group = null ) {
		return new WRMB2_Field( $this->get_default_args( $field_args, $field_group ) );
	}

	/**
	 * Determine whether this wrmb object should show, based on the 'show_on_cb' callback.
	 *
	 * @since 2.0.9
	 *
	 * @return bool Whether this wrmb should be shown.
	 */
	public function should_show() {
		// Default to showing this wrmb.
		$show = true;

		// Use the callback to determine showing the wrmb, if it exists.
	...
	public function maybe_hook_parameter( $hook_name, $val = null, $hook_function = 'add_filter' ) {
		...
		// phpcs:ignore
		$parameter = substr( $hook_name, strlen( 'wrmb2_api_' ) ) . '_cb';
	 ...
	 * @param string $version  The version of WRMB2 that deprecated the argument used.
	...
	protected function deprecated_param( $function, $version, $message = null ) {
		...
		if ( is_numeric( $message ) ) {
			...
			switch ( $message ) {
				...
				case self::DEPRECATED_PARAM:
					/* translators: %1$s is replaced with the attribute, %2$s is replaced with the attribute */
					$message = sprintf( __( 'The "%1$s" field parameter has been deprecated in favor of the "%2$s" parameter.', 'wrmb2' ), $args[3], $args[4] );
					break;

				case self::DEPRECATED_CB_PARAM:
					/* translators: %1$s is replaced with the attribute, %2$s is replaced with the attribute */
					$message = sprintf( __( 'Using the "%1$s" field parameter as a callback has been deprecated in favor of the "%2$s" parameter.', 'wrmb2' ), $args[3], $args[4] );
					break;
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG && apply_filters( 'deprecated_argument_trigger_error', true ) ) {
			if ( function_exists( '__' ) ) {
				if ( ! is_null( $message ) ) {
					/* translators: %1$s is replaced with the function, %2$s is replaced with the version, %3$s is replaced with the message */
					trigger_error( sprintf( esc_attr__( '%1$s was called with a parameter that is <strong>deprecated</strong> since version %2$s! %3$s', 'wrmb2' ), esc_attr( $function ), esc_attr( $version ), esc_attr( $message ) ) );
				} else {
					/* translators: %1$s is replaced with the function, %2$s is replaced with the version */
					trigger_error( sprintf( esc_attr__( '%1$s was called with a parameter that is <strong>deprecated</strong> since version %2$s with no alternative available.', 'wrmb2' ), esc_attr( $function ), esc_attr( $version ) ) );
				}
			} elseif ( ! is_null( $message ) ) {
					/* translators: %1$s is replaced with the function, %2$s is replaced with the version, %3$s is replaced with the message */
					trigger_error( sprintf( '%1$s was called with a parameter that is <strong>deprecated</strong> since version %2$s! %3$s', esc_attr( $function ), esc_attr( $version ), esc_attr( $message ) ) );
			} else {
				/* translators: %1$s is replaced with the function, %2$s is replaced with the version */
				trigger_error( sprintf( '%1$s was called with a parameter that is <strong>deprecated</strong> since version %2$s with no alternative available.', esc_attr( $function ), esc_attr( $version ) ) );
			}
		}
	}
	...
	public function __call( $method, $args ) {
		...
		if ( ! has_filter( "{$object_class}_inherit_{$method}" ) ) {
			/* translators: %1$s is replaced with the class, %2$s is replaced with the method */
			throw new Exception( sprintf( esc_html__( 'Invalid %1$s method: %2$s', 'wrmb2' ), esc_attr( get_class( $this ) ), esc_attr( $method ) ) );
		}

		array_unshift( $args, $this );

		/**
		 * Allows overloading the object (WRMB2 or WRMB2_Field) with additional capabilities
		 * by registering hook callbacks.
		 *
		 * The first dynamic portion of the hook name, $object_class, refers to the object class,
		 * either wrmb2 or wrmb2_field.
		 *
		 ...
		 * use a unique method (aka, $wrmb->prefix_my_method()).
		 *
