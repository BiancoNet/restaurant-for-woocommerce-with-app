...
if ( ! class_exists( 'WP_REST_Controller' ) ) {
	...
	require_once wrmb2_dir( 'includes/shim/class-wp-rest-controller.php' );
}

/**
 * Creates WRMB2 objects/fields endpoint for WordPress REST API.
 ...
 * @link      https://wrmb2.io
 */
abstract class WRMB2_REST_Controller extends WP_REST_Controller {
	 ...
	 * @var string
	 */
	protected $namespace = WRMB2_REST::NAME_SPACE;
	 ...
	 * Constructor.
	 *
	 * @param WP_REST_Server $wp_rest_server it is the rest server.
	 * @since 2.2.3
	 */
	 ...
	 *
	 * Checks if a WRMB object callback property exists, and if it does,
	...
	public function maybe_hook_callback_and_apply_filters( $filter, $default_access ) {
		if ( ! $this->rest_box && $this->request->get_param( 'wrmb_id' ) ) {
			$this->rest_box = WRMB2_REST::get_rest_box( $this->request->get_param( 'wrmb_id' ) );
		}
		 ...
		 * @since 2.2.3
		 *
		 * @param bool   $default_access Whether this WRMB2 endpoint can be accessed.
	...
	/**
	 * Checks if the WRMB2 box has any registered callback parameters for the given filter.
	 *
	...
	public function maybe_hook_registered_callback( $filter, $default_val ) {
		...
		$val = $this->rest_box->wrmb->maybe_hook_parameter( $filter, $default_val );
		if ( null !== $val ) {
			$default_val = $val;
		}
	...
	/**
	 * Unhooks any WRMB2 box registered callback parameters for the given filter.
	 *
	 * @since  2.2.3
	 *
	 * @param  string $filter The filter name.
	...
	public function maybe_unhook_registered_callback( $filter ) {
		...
		$this->rest_box->wrmb->maybe_hook_parameter( $filter, null, 'remove_filter' );
	}

	/**
	 * Prepare a WRMB2 object for serialization
	 *
	 * @since 2.2.3
	 *
	 * @param  mixed $data it is the data.
	 * @return array $data
	 */
	...
	/**
	 * Prepare the WRMB2 item for the REST response.
	 *
	 * @since 2.2.3
	 *
	...
	public function prepare_item_for_response( $data, $request = null ) {
		...
		/**
		 * Filter the prepared WRMB2 item response.
		 *
		 * @since 2.2.3
		 *
		 * @param mixed  $data           Prepared data
		 * @param object $request        The WP_REST_Request object
		 * @param object $wrmb2_endpoints This endpoints object
		 */
		return apply_filters( 'wrmb2_rest_prepare', rest_ensure_response( $data ), $this->request, $this );
	}
	...
	protected function initiate_rest_read_box( $request, $request_type ) {
		...
		if ( ! is_wp_error( $this->rest_box ) && ! $this->rest_box->rest_read ) {
			$this->rest_box = new WP_Error(
				'wrmb2_rest_no_read_error',
				__( 'This box does not have read permissions.', 'wrmb2' ),
				array(
					'status' => 403,
				)
			);
	...
	protected function initiate_rest_edit_box( $request, $request_type ) {
		...
		if ( ! is_wp_error( $this->rest_box ) && ! $this->rest_box->rest_edit ) {
			$this->rest_box = new WP_Error(
				'wrmb2_rest_no_write_error',
				__( 'This box does not have write permissions.', 'wrmb2' ),
				array(
					'status' => 403,
				)
			);
	...
	protected function initiate_rest_box( $request, $request_type ) {
		...
		if ( ! $this->rest_box ) {
			...
			$this->rest_box = new WP_Error(
				'wrmb2_rest_box_not_found_error',
				__( 'No box found by that id. A box needs to be registered with the "show_in_rest" parameter configured.', 'wrmb2' ),
				array(
					'status' => 403,
				)
			);
			...
			if ( isset( $this->request['object_id'] ) ) {
				$this->rest_box->wrmb->object_id( sanitize_text_field( $this->request['object_id'] ) );
			}

			if ( isset( $this->request['object_type'] ) ) {
				$this->rest_box->wrmb->object_type( sanitize_text_field( $this->request['object_type'] ) );
			}
		}
	}
	...
	/**
	 * Get WRMB2 fields schema, conforming to JSON Schema
	 *
	 * @since 2.2.3
	 *
	 * @return array
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				'description' => array(
					'description' => __( 'A human-readable description of the object.', 'wrmb2' ),
					'type'        => 'string',
					'context'     => array(
						'view',
					),
				...
				'name'        => array(
					'description' => __( 'The title for the object.', 'wrmb2' ),
					'type'        => 'string',
					'context'     => array(
						'view',
					),
	...
	public function get_query_string() {
		...
		foreach ( $defaults as $key => $value ) {
			if ( isset( $this->request[ $key ] ) ) {
				...
				$query_string .= $key;
				$value         = sanitize_text_field( $this->request[ $key ] );
				if ( $value ) {
					$query_string .= '=' . $value;
				}
			}
