<?php
/**
 * It is the REST Controller Fields.
 *
 * @package woorestaurant
 */

/**
 * WRMB2 objects/fields endpoint for WordPress REST API.
 ...
 * @link      https://wrmb2.io
 */
class WRMB2_REST_Controller_Fields extends WRMB2_REST_Controller_Boxes {
	...
	public function register_routes() {
		$args = array(
			'_embed'      => array(
				'description' => __( 'Includes the box object which the fields are registered to in the response.', 'wrmb2' ),
			),
			'_rendered'   => array(
				'description' => __( 'When the \'_rendered\' argument is passed, the renderable field attributes will be returned fully rendered. By default, the names of the callback handers for the renderable attributes will be returned.', 'wrmb2' ),
			),
			'object_id'   => array(
				'description' => __( 'To view or modify the field\'s value, the \'object_id\' and \'object_type\' arguments are required.', 'wrmb2' ),
			),
			'object_type' => array(
				'description' => __( 'To view or modify the field\'s value, the \'object_id\' and \'object_type\' arguments are required.', 'wrmb2' ),
			),
		...
		register_rest_route(
			$this->namespace,
			'/' . $this->rest_base . '/(?P<wrmb_id>[\w-]+)/fields/',
			array(
				array(
		...
		register_rest_route(
			$this->namespace,
			'/' . $this->rest_base . '/(?P<wrmb_id>[\w-]+)/fields/(?P<field_id>[\w-]+)',
			array(
				array(
					'methods'             => WP_REST_Server::READABLE,
	...
	public function get_items_permissions_check( $request ) {
		 ...
		 * @since 2.2.3
		 *
		 * @param bool   $can_access Whether this WRMB2 endpoint can be accessed.
		 * @param object $controller This WRMB2_REST_Controller object.
		 */
		return $this->maybe_hook_callback_and_apply_filters( 'wrmb2_api_get_fields_permissions_check', $can_access );
	}

	/**
	 * Get all public WRMB2 box fields.
	 *
	 * @since 2.2.3
	 *
	 * @param  WP_REST_Request $request Full data about the request.
	...
	public function get_items( $request ) {
		...
		$fields = array();
		foreach ( $this->rest_box->wrmb->prop( 'fields', array() ) as $field ) {
	...
	public function get_item_permissions_check_filter( $can_access = true ) {
		 ...
		 * @param object $controller This WRMB2_REST_Controller object.
		 */
		return $this->maybe_hook_callback_and_apply_filters( 'wrmb2_api_get_field_permissions_check', $can_access );
	}

	/**
	 * Get one WRMB2 field from the collection.
	 *
	 * @since 2.2.3
	 *
	 * @param  WP_REST_Request $request Full data about the request.
	...
	public function update_item_permissions_check( $request ) {
		 ...
		 * @since 2.2.3
		 *
		 * @param bool   $can_update Whether this WRMB2 endpoint can be accessed.
		 * @param object $controller This WRMB2_REST_Controller object.
		 */
		return $this->maybe_hook_callback_and_apply_filters( 'wrmb2_api_update_field_value_permissions_check', $can_update );
	}

	/**
	 * Update WRMB2 field value.
	 *
	 * @since 2.2.3
	 *
	 * @param  WP_REST_Request $request Full data about the request.
	...
	public function update_item( $request ) {
		...
		if ( ! $this->request['value'] ) {
			return new WP_Error(
				'wrmb2_rest_update_field_error',
				__( 'WRMB2 Field value cannot be updated without the value parameter specified.', 'wrmb2' ),
				array(
					'status' => 400,
				)
			);
	...
	public function delete_item_permissions_check( $request ) {
		 ...
		 * @since 2.2.3
		 *
		 * @param bool   $can_delete Whether this WRMB2 endpoint can be accessed.
		 * @param object $controller This WRMB2_REST_Controller object.
		 */
		return $this->maybe_hook_callback_and_apply_filters( 'wrmb2_api_delete_field_value_permissions_check', $can_delete );
	}

	/**
	 * Delete WRMB2 field value.
	 *
	 * @since 2.2.3
	 *
	 * @param  WP_REST_Request $request Full data about the request.
	...
	public function modify_field_value( $activity ) {
		...
		if ( ! $this->request['object_id'] || ! $this->request['object_type'] ) {
			return new WP_Error(
				'wrmb2_rest_modify_field_value_error',
				__( 'WRMB2 Field value cannot be modified without the object_id and object_type parameters specified.', 'wrmb2' ),
				array(
					'status' => 400,
				)
			);
		...
		if ( ! $this->field ) {
			return new WP_Error(
				'wrmb2_rest_no_field_by_id_error',
				__( 'No field found by that id.', 'wrmb2' ),
				array(
					'status' => 403,
				)
			);
		...
		if ( 'options-page' == $this->request['object_type'] ) {
			$this->field->args[ "value_{$activity}" ] = wrmb2_options( $this->request['object_id'] )->set();
		}
	 ...
	 * @since 2.2.3
	 *
	 * @param  string $field Field id or Field object.
	 * @return WP_Error|WP_REST_Response
	 */
	 ...
	 */
	protected function prepare_field_data( WRMB2_Field $field ) {
		$field_data       = array();
		...
		foreach ( $field_args as $key => $value ) {
			...
			if ( 'options_cb' === $key ) {
				$value = $field->options();
			} elseif ( in_array( $key, WRMB2_Field::$callable_fields, true ) ) {

				if ( isset( $this->request['_rendered'] ) ) {
					$value = 'render_row_cb' === $key ? $rendered : $field->get_param_callback_result( $key );
			...
			if ( empty( $value ) || is_scalar( $value ) || is_array( $value ) ) {
				...
				/* translators: %s is replaced with the key */
				$field_data[ $key ] = sprintf( __( 'Value Error for %s', 'wrmb2' ), $key );
			}
		}
	 ...
	 * @since  2.2.3
	 *
	 * @param  WRMB2_Field $field Field object to build links from.
	 *
	 * @return array             Array of links
	 */
	protected function prepare_links( $field ) {
		$boxbase      = $this->namespace_base . '/' . $this->rest_box->wrmb->wrmb_id;
	...
	/**
	 * Checks if the WRMB2 box or field has any registered callback parameters for the given filter.
	 *
	 * The registered handlers will have a property name which matches the filter, except:
	 * - The 'wrmb2_api' prefix will be removed
	...
	/**
	 * Unhooks any WRMB2 box or field registered callback parameters for the given filter.
	 *
	 * @since  2.2.3
	 *
	 * @param  string $filter The filter name.
