<?php
/**
 * It is the REST Controller Boxes.
 *
 * @package woorestaurant
 */

/**
 * WRMB2 objects/boxes endpoint for WordPress REST API.
 ...
 * @link      https://wrmb2.io
 */
class WRMB2_REST_Controller_Boxes extends WRMB2_REST_Controller {
	...
	public function register_routes() {
		$args = array(
			'_embed' => array(
				'description' => __( 'Includes the registered fields for the box in the response.', 'wrmb2' ),
			),
		);
		...
		$args['_rendered'] = array(
			'description' => __( 'Includes the fully rendered attributes, \'form_open\', \'form_close\', as well as the enqueued \'js_dependencies\' script handles, and \'css_dependencies\' stylesheet handles.', 'wrmb2' ),
		);

		// Returns specific box's data.
		register_rest_route(
			$this->namespace,
			'/' . $this->rest_base . '/(?P<wrmb_id>[\w-]+)',
			array(
				array(
					'methods'             => WP_REST_Server::READABLE,
	...
	public function get_items_permissions_check( $request ) {
		 ...
		 */
		return apply_filters( 'wrmb2_api_get_boxes_permissions_check', true, $this );
	}

	/**
	 * Get all public WRMB2 boxes.
	 *
	 * @since 2.2.3
	 *
	 * @param  WP_REST_Request $request Full data about the request.
	...
	public function get_items( $request ) {
		...
		$boxes = WRMB2_REST::get_all();
		if ( empty( $boxes ) ) {
			return new WP_Error(
				'wrmb2_rest_no_boxes',
				__( 'No boxes found.', 'wrmb2' ),
				array(
					'status' => 403,
				)
			);
	...
	public function get_item_permissions_check_filter( $can_access = true ) {
		 ...
		 * @param object $controller This WRMB2_REST_Controller object.
		 */
		return $this->maybe_hook_callback_and_apply_filters( 'wrmb2_api_get_box_permissions_check', $can_access );
	}

	/**
	 * Get one WRMB2 box from the collection.
	 *
	 * @since 2.2.3
	 *
	 * @param  WP_REST_Request $request Full data about the request.
	...
	/**
	 * Get a WRMB2 box prepared for REST
	 *
	 * @since 2.2.3
	 *
	 * @return array
	...
	public function get_rest_box() {
		$wrmb = $this->rest_box->wrmb;

		$boxes_data = $wrmb->meta_box;

		if ( isset( $this->request['_rendered'] ) && ltrim( WRMB2_REST_Controller::get_intial_route(), '/' ) !== $this->namespace_base ) {
			$boxes_data['form_open']  = $this->get_cb_results( array( $wrmb, 'render_form_open' ) );
			$boxes_data['form_close'] = $this->get_cb_results( array( $wrmb, 'render_form_close' ) );
		...
		$response->add_links( $this->prepare_links( $wrmb ) );
	 ...
	 * @since  2.2.3
	 *
	 * @param  WRMB2_REST $wrmb WRMB2_REST object to build links from.
	 *
	 * @return array          Array of links
	 */
	protected function prepare_links( $wrmb ) {
		$boxbase      = $this->namespace_base . '/' . $wrmb->wrmb_id;
