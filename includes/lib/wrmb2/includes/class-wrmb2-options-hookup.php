<?php
/**
 * It is the WRMB2 options hookup file.
 *
 ...
 * @link      https://wrmb2.io
 */
class WRMB2_Options_Hookup extends WRMB2_Hookup {
	 ...
	 */
	public function __construct( WRMB2 $wrmb, $option_key ) {
		$this->wrmb       = $wrmb;
		$this->option_key = $option_key;
	}

	/**
	 * It is the Hooks method.
	 */
	public function hooks() {
		...
		if ( ! $this->wrmb->prop( 'autoload', true ) ) {
			// Disable option autoload if requested.
			add_filter( "wrmb2_should_autoload_{$this->option_key}", '__return_false' );
		}

		/**
		 * For WP < 4.7. Ensure the register_setting function exists.
		 */
		if ( ! WRMB2_Utils::wp_at_least( '4.7' ) && ! function_exists( 'register_setting' ) ) {
			require_once ABSPATH . 'wp-admin/includes/plugin.php';
		}

		// Register setting to wrmb2 group.
		register_setting( 'wrmb2', $this->option_key );

		// Handle saving the data.
		add_action( 'admin_post_' . $this->option_key, array( $this, 'save_options' ) );

		// Optionally network_admin_menu.
		$hook = $this->wrmb->prop( 'admin_menu_hook' );
		...
		if ( 'network_admin_menu' === $hook ) {
			// Override WRMB's getter.
			add_filter( "wrmb2_override_option_get_{$this->option_key}", array( $this, 'network_get_override' ), 10, 2 );
			// Override WRMB's setter.
			add_filter( "wrmb2_override_option_save_{$this->option_key}", array( $this, 'network_update_override' ), 10, 2 );
		}
	}
	...
	public function options_page_menu_hooks() {
		$parent_slug = $this->wrmb->prop( 'parent_slug' );
		$title       = $this->wrmb->prop( 'title' );
		$menu_title  = $this->wrmb->prop( 'menu_title', $title );
		$capability  = $this->wrmb->prop( 'capability' );
		...
		if ( $parent_slug ) {
			...
			$page_hook = add_menu_page(
				...
				$this->option_key,
				$callback,
				$this->wrmb->prop( 'icon_url' ),
				$this->wrmb->prop( 'position' )
			);
		}

		if ( $this->wrmb->prop( 'wrmb_styles' ) ) {
			// Include WRMB CSS in the head to avoid FOUC.
			add_action( "admin_print_styles-{$page_hook}", array( 'WRMB2_Hookup', 'enqueue_wrmb_css' ) );
		}
	...
	public function maybe_register_message() {
		...
		$should_notify   = ! $this->wrmb->prop( 'disable_settings_errors' ) && isset( $_GET['settings-updated'] ) && $is_options_page;
		$is_updated      = $should_notify && 'true' === $_GET['settings-updated'];
		$setting         = "{$this->option_key}-notices";
		$code            = '';
		$message         = __( 'Nothing to update.', 'wrmb2' );
		$type            = 'notice-warning';

		if ( $is_updated ) {
			$message = __( 'Settings updated.', 'wrmb2' );
			$type    = 'updated';
		}

		// Check if parameter has registered a callback.
		$cb = $this->wrmb->maybe_callback( 'message_cb' );
		if ( $cb ) {
	...
	public function options_page_output() {
		$this->maybe_output_settings_notices();
			...
			$callback = $this->wrmb->prop( 'display_cb' );
		...
		?>
		<div class="wrap wrmb2-options-page option-<?php echo esc_attr( sanitize_html_class( $this->option_key ) ); ?>">
			<?php if ( $this->wrmb->prop( 'title' ) ) : ?>
				<h2><?php echo wp_kses_post( $this->wrmb->prop( 'title' ) ); ?></h2>
			<?php endif; ?>
			<?php if ( ! empty( $tabs ) ) : ?>
			...
			<?php endif; ?>
			<form class="wrmb-form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="POST" id="<?php echo esc_attr( $this->wrmb->wrmb_id ); ?>" enctype="multipart/form-data" encoding="multipart/form-data">
				<input type="hidden" name="action" value="<?php echo esc_attr( $this->option_key ); ?>">
				<?php $this->options_page_metabox(); ?>
				<?php submit_button( esc_attr( $this->wrmb->prop( 'save_button' ) ), 'primary', 'submit-wrmb' ); ?>
			</form>
		</div>
		<?php
	}
	...
	/**
	 * Gets navigation tabs array for WRMB2 options pages which share the
	...
	public function get_tab_group_tabs() {
		$tab_group = $this->wrmb->prop( 'tab_group' );
		$tabs      = array();

		if ( $tab_group ) {
			$boxes = WRMB2_Boxes::get_by( 'tab_group', $tab_group );

			foreach ( $boxes as $wrmb_id => $wrmb ) {
				$option_key = $wrmb->options_page_keys();

				// Must have an option key, must be an options page box.
				if ( ! isset( $option_key[0] ) || 'options-page' !== $wrmb->mb_object_type() ) {
					continue;
				}

				$tabs[ $option_key[0] ] = $wrmb->prop( 'tab_title', $wrmb->prop( 'title' ) );
			}
		}
	...
	public function save_options() {
		if ( isset( $_POST['token'] ) ) {
			// phpcs:ignore
			if ( wp_verify_nonce( wp_kses_post( wp_unslash( $_POST['token'] ) ), 'save-options' ) ) {
				return;
			}
		}
		...
		if (
			...
			// check params.
			&& isset( $_POST['submit-wrmb'], $_POST['action'] )
			&& $this->option_key === $_POST['action']
		) {

			$updated = $this->wrmb
				->save_fields( $this->option_key, $this->wrmb->object_type(), $_POST )
