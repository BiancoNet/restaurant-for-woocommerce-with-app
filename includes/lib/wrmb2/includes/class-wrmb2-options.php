<?php
/**
 * It is the WRMB2 Options.
 *
 * @package woorestaurant
 */

/**
 * WRMB2 Utility classes for handling multi-dimensional array data for options
 *
...
/**
 * Retrieves an instance of WRMB2_Option based on the option key
 *
 * @package   WRMB2
 * @author    WRMB2 team
 */
class WRMB2_Options {
	/**
	 * Array of all WRMB2_Option instances
	 *
	 * @var   array
	 * @since 1.0.0
	 */
	 ...
	 *
	 * @param string $option_key It is the option key.
	 */
	public static function get( $option_key ) {

		if ( empty( self::$option_sets ) || empty( self::$option_sets[ $option_key ] ) ) {
			self::$option_sets[ $option_key ] = new WRMB2_Option( $option_key );
		}
...
class WRMB2_Option {
	 ...
	 *
	 * @uses apply_filters() Calls 'wrmb2_override_option_save_{$this->key}' hook
	...
	public function set( $options = array() ) {
		...
		$test_save = apply_filters( "wrmb2_override_option_save_{$this->key}", 'wrmb2_no_override_option_save', $this->options, $this );

		if ( 'wrmb2_no_override_option_save' !== $test_save ) {
		 ...
		 * @param WRMB2_Option $wrmb_option This object.
		 */
		$autoload = apply_filters( "wrmb2_should_autoload_{$this->key}", true, $this );
	 ...
	 *
	 * @uses apply_filters() Calls 'wrmb2_override_option_get_{$this->key}' hook to allow
	...
	public function get_options( $default = null ) {
		if ( empty( $this->options ) && ! empty( $this->key ) ) {
			...
			$test_get = apply_filters( "wrmb2_override_option_get_{$this->key}", 'wrmb2_no_override_option_get', $default, $this );

			if ( 'wrmb2_no_override_option_get' !== $test_get ) {
