<?php
/**
 * It is the WRMB2 file.
 *
 * @package woorestaurant
 */

/**
 * WRMB2 - The core metabox object
 *
...
/**
 * The main WRMB2 object for storing box data/properties.
 */
class WRMB2 extends WRMB2_Base {
	...
	protected $mb_defaults = array(
		...
		'wrmb_styles'             => true, // Include WRMB2 stylesheet.
		'enqueue_js'              => true, // Include WRMB2 JS.
		'fields'                  => array(),

		/**
		 * Handles hooking WRMB2 forms/metaboxes into the post/attachement/user/options-page screens
		...
		'show_in_rest'            => false,
		'classes'                 => null, // Optionally add classes to the WRMB2 wrapper.
		'classes_cb'              => '', // Optionally add classes to the WRMB2 wrapper (via a callback).
		 ...
		 *
		 * WRMB2 does not use these arguments in the add_meta_box callback, however, these args
		...
		'display_cb'              => false, // Override the options-page form output (WRMB2_Hookup::options_page_output()).
	...
	/**
	 * Whether there are fields to be shown in columns. Set in WRMB2::add_field().
	 *
	 * @var   bool
	 * @since 2.2.2
	 */
	...
	public function __construct( $config, $object_id = 0 ) {
		...
		if ( empty( $config['id'] ) ) {
			wp_die( esc_html__( 'Metabox configuration is required to have an ID parameter.', 'wrmb2' ) );
		}

		$this->wrmb_id            = $config['id'];
		...
		WRMB2_Boxes::add( $this );

		/**
		 * Hook during initiation of WRMB2 object
		 *
		 * The dynamic portion of the hook name, $this->wrmb_id, is this meta_box id.
		 *
		 * @param array $wrmb This WRMB2 object
		 */
		do_action( "wrmb2_init_{$this->wrmb_id}", $this );

		// Hook in the hookup... how meta.
		add_action( "wrmb2_init_hookup_{$this->wrmb_id}", array( 'WRMB2_Hookup', 'maybe_init_and_hookup' ) );

		// Hook in the rest api functionality.
		add_action( "wrmb2_init_hookup_{$this->wrmb_id}", array( 'WRMB2_REST', 'maybe_init_and_hookup' ) );
	}
	 ...
	 * 'wrmb2_before_form' and "wrmb2_before_{$object_type}_form_{$this->wrmb_id}"
	 *
	 * @since  2.2.0
	...
	public function render_form_open( $object_id = 0, $object_type = '' ) {
		 ...
		 *
		 * @param array  $wrmb_id      The current box ID.
		 ...
		 * @param array  $wrmb         This WRMB2 object.
		 */
		do_action( 'wrmb2_before_form', $this->wrmb_id, $object_id, $object_type, $this );
		 ...
		 * Could also be `comment`, `user` or `options-page`.
		 *
		 * The second dynamic portion of the hook name, $this->wrmb_id, is the meta_box id.
		 *
		 * @param array  $wrmb_id      The current box ID
		 * @param int    $object_id   The ID of the current object
		 * @param array  $wrmb         This WRMB2 object
		 */
		do_action( "wrmb2_before_{$object_type}_form_{$this->wrmb_id}", $object_id, $this );

		echo '<div class="', wp_kses_post( $this->box_classes() ), '">
		<div id="wrmb2-metabox-', sanitize_html_class( $this->wrmb_id ), '" class="wrmb2-metabox wrmb-field-list">';

		return $this;
	}

	/**
	 * Defines the classes for the WRMB2 form/wrap.
	 *
	 * @since  2.0.0
	...
	public function box_classes() {
		...
		$classes = array( 'wrmb2-wrap', 'form-table' );
		...
		if ( $this->is_alternate_context_box() ) {
			...
			if ( ! $this->prop( 'title' ) && ! $this->prop( 'remove_box_wrap' ) ) {
				$context[] = 'wrmb2-context-wrap-no-title';
			}

			// Include a generic context wrapper.
			$context[] = 'wrmb2-context-wrap';

			// Include a context-type based context wrapper.
			$context[] = 'wrmb2-context-wrap-' . $this->prop( 'context' );

			// Include an ID based context wrapper as well.
			$context[] = 'wrmb2-context-wrap-' . $this->prop( 'id' );
		 ...
		 * @since 2.2.2
		 *
		 * @param string $classes Array of classes for the wrmb2-wrap.
		 * @param WRMB2   $wrmb     This WRMB2 object.
		 */
		$classes = apply_filters( 'wrmb2_wrap_classes', $classes, $this );
	 ...
	 * 'wrmb2_after_form' and "wrmb2_after_{$object_type}_form_{$this->wrmb_id}"
	 *
	 * @since  2.2.0
	 * @param  integer $object_id   Object ID.
	 * @param  string  $object_type Object type.
	...
	public function render_form_close( $object_id = 0, $object_type = '' ) {
		 ...
		 *
		 * The dynamic portion of the hook name, $this->wrmb_id, is the meta_box id.
		 *
		 * The first dynamic portion of the hook name, $object_type, is the type of object
		 * you are working with. Usually `post` (this applies to all post-types).
		 * Could also be `comment`, `user` or `options-page`.
		 ...
		 * @param array  $wrmb         This WRMB2 object
		 */
		do_action( "wrmb2_after_{$object_type}_form_{$this->wrmb_id}", $object_id, $this );
		 ...
		 * @param array  $wrmb         This WRMB2 object.
		 */
		do_action( 'wrmb2_after_form', $this->wrmb_id, $object_id, $object_type, $this );
	 ...
	 * @return mixed WRMB2_Field object if successful.
	 */
	 ...
	 *
	 * @param array|WRMB2_Field $args Array of field arguments for a group field parent or the group parent field.
	 * @return WRMB2_Field|null Group field object.
	 */
	public function render_group( $args ) {
		$field_group = false;

		if ( $args instanceof WRMB2_Field ) {
	 ...
	 * @param  WRMB2_Field $field_group The WRMB2_Field group object.
	 *
	 * @return WRMB2_Field|null Group field object.
	 */
	public function render_group_callback( $field_args, $field_group ) {
		...
		echo '<div class="wrmb-row wrmb-repeat-group-wrap ', wp_kses_post( $field_group->row_classes() ), '" data-fieldtype="group"><div class="wrmb-td" style="width: 100%;"><div data-groupid="', wp_kses_post( $field_group->id() ), '" id="', wp_kses_post( $field_group->id() ), '_repeat" ', wp_kses_post( $this->group_wrap_attributes( $field_group ) ), '>';

		if ( $desc || $label ) {
			$class = $desc ? ' wrmb-group-description' : '';
			echo '<div class="wrmb-row', wp_kses_post( $class ), '"><div class="wrmb-th">';
			if ( $label ) {
				echo '<h2 class="wrmb-group-name">', wp_kses_post( $label ), '</h2>';
			}
			if ( $desc ) {
				echo '<p class="wrmb2-metabox-description">', wp_kses_post( $desc ), '</p>';
			}
			echo '</div></div>';
		}
		...
		if ( $field_group->args( 'repeatable' ) ) {
			echo '<div class="wrmb-row"><div class="wrmb-td"><p class="wrmb-add-row"><button type="button" data-selector="', wp_kses_post( $field_group->id() ), '_repeat" data-grouptitle="', wp_kses_post( $field_group->options( 'group_title' ) ), '" class="wrmb-add-group-row button-secondary">', wp_kses_post( $field_group->options( 'add_button' ) ), '</button></p></div></div>';
		}
	 ...
	 *
	 * @since  2.2.3
	 * @param  WRMB2_Field $field_group The group WRMB2_Field object.
	 * @return string                  The attributes string.
	 */
	public function group_wrap_attributes( $field_group ) {
		$classes  = 'wrmb-nested wrmb-field-list wrmb-repeatable-group';
		 ...
		 * @param WRMB2_Field $field_group           The group WRMB2_Field object.
		 */
		$group_wrap_attributes = apply_filters( 'wrmb2_group_wrap_attributes', $group_wrap_attributes, $field_group );
	
		$atts = array();
		foreach ( $group_wrap_attributes as $att => $att_value ) {
			if ( ! WRMB2_Utils::is_data_attribute( $att ) ) {
				// Explicitly set the $flags parameter for compatibility with PHP 8.1 and earlier
				$att_value = htmlspecialchars( $att_value, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 );
			}
	
			$atts[ sanitize_html_class( $att ) ] = sanitize_text_field( $att_value );
		}
	
		return WRMB2_Utils::concat_attrs( $atts );
	}
	 ...
	 *
	 * @since  1.0.2
	 * @param  WRMB2_Field $field_group     WRMB2_Field group field object.
	 *
	 * @return WRMB2
	 */
	public function render_group_row( $field_group ) {
		...
		echo '
		<div id="wrmb-group-', wp_kses_post( $field_group->id() ), '-', wp_kses_post( $field_group->index ), '" class="postbox wrmb-row wrmb-repeatable-grouping', wp_kses_post( $closed_class ), '" data-iterator="', wp_kses_post( $field_group->index ), '">';

		if ( $field_group->args( 'repeatable' ) ) {
			echo '<button type="button" data-selector="', wp_kses_post( $field_group->id() ), '_repeat" data-confirm="', wp_kses_post( $confirm_deletion ), '" class="dashicons-before dashicons-no-alt wrmb-remove-group-row" title="', wp_kses_post( $field_group->options( 'remove_button' ) ), '"></button>';
		}

			echo '
			<div class="wrmbhandle" title="', esc_attr__( 'Click to toggle', 'wrmb2' ), '"><br></div>
			<h3 class="wrmb-group-title wrmbhandle-title"><span>', wp_kses_post( $field_group->replace_hash( $field_group->options( 'group_title' ) ) ), '</span></h3>

			<div class="inside wrmb-td wrmb-nested wrmb-field-list">';
		...
		if ( $field_group->args( 'repeatable' ) ) {
			echo '
					<div class="wrmb-row wrmb-remove-field-row">
						<div class="wrmb-remove-row">
							<button type="button" data-selector="', wp_kses_post( $field_group->id() ), '_repeat" data-confirm="', wp_kses_post( $confirm_deletion ), '" class="wrmb-remove-group-row wrmb-remove-group-row-button alignright button-secondary">', wp_kses_post( $field_group->options( 'remove_button' ) ), '</button>
						</div>
					</div>
					';
		}
	 ...
	 * @since 2.0.0
	 *
	 * @param array            $field_args  Array of field arguments to be passed to WRMB2_Field.
	 * @param WRMB2_Field|null $field_group WRMB2_Field group field object.
	 * @return WRMB2_Field
	 */
	public function add_hidden_field( $field_args, $field_group = null ) {
		if ( isset( $field_args['field_args'] ) ) {
			// For back-compatibility.
			$field = new WRMB2_Field( $field_args );
		} else {
			$field = $this->get_new_field( $field_args, $field_group );
		}

		$types = new WRMB2_Types( $field );
	...
	public function get_sanitized_values( array $data_to_sanitize ) {
		...
		// We do this So WRMB will sanitize our data for us, but not save it.
		$this->object_id( '_' );
		...
		// phpcs:ignore
		add_filter( "wrmb2_return_taxonomy_values_{$this->wrmb_id}", '__return_true' );
		...
		// phpcs:ignore
		remove_filter( "wrmb2_return_taxonomy_values_{$this->wrmb_id}", '__return_true' );

		// Get data from temp. data store.
		$sanitized_values = wrmb2_options( 0 )->get_options();
	...
	public function save_fields( $object_id = 0, $object_type = '', $data_to_save = array() ) {
		if ( isset( $_POST['token'] ) ) {
			// phpcs:ignore
			if ( wp_verify_nonce( wp_kses_post( wp_unslash( $_POST['token'] ) ), 'save-fields' ) ) {
				return;
			}
		}
	...
	/**
	 * Fires the "wrmb2_{$object_type}_process_fields_{$wrmb_id}" action hook.
	 *
	 * @since 2.2.2
	 *
	 * @return WRMB2
	...
	public function pre_process() {
		 ...
		 * @param int   $object_id The ID of the current object
		 */
		do_action( "wrmb2_{$object_type}_process_fields_{$this->wrmb_id}", $this, $this->object_id() );

		return $this;
	}

	/**
	 * Fires the "wrmb2_save_{$object_type}_fields" and
	 * "wrmb2_save_{$object_type}_fields_{$wrmb_id}" action hooks.
	 *
	 * @since  2.x.x
	 *
	 * @return WRMB2
	...
	public function after_save() {
		 ...
		 * @param array  $wrmb         This WRMB2 object
		 */
		do_action( "wrmb2_save_{$object_type}_fields", $object_id, $this->wrmb_id, $this->updated, $this );
		 ...
		 * @param array  $wrmb         This WRMB2 object
		 */
		do_action( "wrmb2_save_{$object_type}_fields_{$this->wrmb_id}", $object_id, $this->updated, $this );
	 ...
	 * @return mixed        Return of WRMB2_Field::update_data().
	 */
	...
	public function save_group_field( $field_group ) {
		...
		foreach ( array_values( $field_group->fields() ) as $field_args ) {
			...
			foreach ( (array) $group_vals as $field_group->index => $post_vals ) {
				...
				$is_updated = ( ! WRMB2_Utils::isempty( $new_val ) && $new_val !== $old_val );
				$is_removed = ( WRMB2_Utils::isempty( $new_val ) && ! WRMB2_Utils::isempty( $old_val ) );
			...
			$saved[ $field_group->index ] = WRMB2_Utils::filter_empty( $saved[ $field_group->index ] );
		}// End foreach.

		$saved = WRMB2_Utils::filter_empty( $saved );
	...
	public function box_types( $fallback = array() ) {
		return WRMB2_Utils::ensure_array( $this->prop( 'object_types' ), $fallback );
	}
	...
	public function is_box_type( $object_types = array(), $fallback = array() ) {
		...
		if ( in_array( 'term', $box_types, true ) ) {
			$taxonomies = WRMB2_Utils::ensure_array( $this->prop( 'taxonomies' ) );
	...
	public function doing_options_page() {
		if ( isset( $_POST['token'] ) ) {
			// phpcs:ignore
			if ( wp_verify_nonce( wp_kses_post( wp_unslash( $_POST['token'] ) ), 'option-page' ) ) {
				return;
			}
		}
		...
		if ( ! empty( $_GET['page'] ) && in_array( $_GET['page'], $keys ) ) {
			$found_key = wp_kses_post( wp_unslash( $_GET['page'] ) );
		}

		if ( ! empty( $_POST['action'] ) && in_array( $_POST['action'], $keys ) ) {
			$found_key = wp_kses_post( wp_unslash( $_POST['action'] ) );
		}
	...
	public function current_object_type() {
		if ( isset( $_POST['token'] ) ) {
			// phpcs:ignore
			if ( wp_verify_nonce( wp_kses_post( wp_unslash( $_POST['token'] ) ), 'object-type' ) ) {
				return;
			}
		}
	 ...
	 *
	 * @since  2.0.3
	 * @param  string|array|WRMB2_Field $field        Metabox field id or field config array or WRMB2_Field object.
	 * @param  WRMB2_Field|null         $field_group  (optional) WRMB2_Field object (group parent).
	 * @param  bool                     $reset_cached (optional) Reset the internal cache for this field object.
	 *                                                Use sparingly.
	 *
	 * @return WRMB2_Field|false                     WRMB2_Field object (or false).
	 */
	public function get_field( $field, $field_group = null, $reset_cached = false ) {
		if ( $field instanceof WRMB2_Field ) {
			return $field;
		}
		...
		$this->fields[ $index ] = new WRMB2_Field( $this->get_field_args( $field_id, $field, $sub_field_id, $field_group ) );

		return $this->fields[ $index ];
	}

	/**
	 * Handles determining which type of arguments to pass to WRMB2_Field
	 *
	 * @since  2.0.7
	 * @param  mixed            $field_id     Field (or group field) ID.
	 * @param  mixed            $field_args   Array of field arguments.
	 * @param  mixed            $sub_field_id Sub field ID (if field_group exists).
	 * @param  WRMB2_Field|null $field_group  If a sub-field, will be the parent group WRMB2_Field object.
	 * @return array                         Array of WRMB2_Field arguments.
	 */
	...
	/**
	 * Get default field arguments specific to this WRMB2 object.
	 *
	 * @since  2.2.0
	 * @param  array       $field_args  Metabox field config array.
	 * @param  WRMB2_Field $field_group (optional) WRMB2_Field object (group parent).
	...
	protected function field_actions( $field ) {
		switch ( $field['type'] ) {
			...
			case 'file_list':
				...
				add_filter( 'wp_prepare_attachment_for_js', array( 'WRMB2_Type_File_Base', 'prepare_image_sizes_for_js' ), 10, 3 );
				break;
			...
			case 'group':
				...
				break;
			case 'colorpicker':
				// https://github.com/JayWood/WRMB2_RGBa_Picker
				// Dequeue the rgba_colorpicker custom field script if it is used,
				// since we now enqueue our own more current version.
				add_action( 'admin_enqueue_scripts', array( 'WRMB2_Type_Colorpicker', 'dequeue_rgba_colorpicker_script' ), 99 );
				break;
		}
	...
	protected function _add_field_to_array( $field, &$fields, $position = 0 ) {
		...
		if ( $position ) {
			WRMB2_Utils::array_insert( $fields, array( $field['id'] => $field ), $position );
		} else {
	...
	/**
	 * Handles metabox property callbacks, and passes this $wrmb object as property.
	 *
	 * @since 2.2.3
	...
	public function nonce() {
		if ( ! $this->generated_nonce ) {
			$this->generated_nonce = sanitize_html_class( 'nonce_' . basename( __FILE__ ) . $this->wrmb_id );
		}
