...
class WRMB2_Ajax {
	...
	protected function __construct() {
		add_action( 'wp_ajax_wrmb2_oembed_handler', array( $this, 'oembed_handler' ) );
		add_action( 'wp_ajax_nopriv_wrmb2_oembed_handler', array( $this, 'oembed_handler' ) );
		// Need to occasionally clean stale oembed cache data from the option value.
		add_action( 'wrmb2_save_options-page_fields', array( __CLASS__, 'clean_stale_options_page_oembeds' ) );
	}
	...
	public function oembed_handler() {
		...
		// Verify our nonce.
		if ( ! ( isset( $_REQUEST['wrmb2_ajax_nonce'], $_REQUEST['oembed_url'] ) && wp_verify_nonce( wp_kses_post( wp_unslash( $_REQUEST['wrmb2_ajax_nonce'] ) ), 'ajax_nonce' ) ) ) {
			die();
		}

		// Sanitize our search string.
		$oembed_string = sanitize_text_field( wp_kses_post( wp_unslash( $_REQUEST['oembed_url'] ) ) );

		// Send back error if empty.
		if ( empty( $oembed_string ) ) {
			wp_send_json_error( '<p class="ui-state-error-text">' . esc_html__( 'Please Try Again', 'wrmb2' ) . '</p>' );
		}
		...
		$html = $this->get_oembed(
			array(
				'url'         => $oembed_url,
				'object_id'   => isset( $_REQUEST['object_id'] ) ? wp_kses_post( wp_unslash( $_REQUEST['object_id'] ) ) : '',
				'object_type' => isset( $_REQUEST['object_type'] ) ? wp_kses_post( wp_unslash( $_REQUEST['object_type'] ) ) : 'post',
				'oembed_args' => $embed_args,
				'field_id'    => isset( $_REQUEST['field_id'] ) ? wp_kses_post( wp_unslash( $_REQUEST['field_id'] ) ) : '',
			)
		);
	...
	public function get_oembed( $args ) {
		...
		if ( $oembed['embed'] && $oembed['embed'] != $oembed['fallback'] ) {
			return '<div class="wrmb2-oembed embed-status">' . $oembed['embed'] . '<p class="wrmb2-remove-wrapper"><a href="#" class="wrmb2-remove-file-button" rel="' . $oembed['args']['field_id'] . '">' . esc_html__( 'Remove Embed', 'wrmb2' ) . '</a></p></div>';
		}
	...
	protected function cache_action( $meta_key ) {
		...
		if ( 'options-page' === $this->object_type ) {
			...
			$status = call_user_func_array( array( wrmb2_options( $this->object_id ), $action ), $args );
		} else {
	...
	public static function clean_stale_options_page_oembeds( $option_key ) {
		$options  = wrmb2_options( $option_key )->get_options();
		$modified = false;
		if ( is_array( $options ) ) {
		...
		if ( $modified ) {
			$updated = wrmb2_options( $option_key )->set( $options );
		}
	}
}
